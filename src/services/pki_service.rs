...
pub async fn handle_enrollment(dto: &EnrollDTO, crypto: &Crypto) -> Result<String, String> {
    ...
    let certificate = sign(intermediate.csr, crypto).await.map_err(|e| {
        format!(
...
pub async fn verify_cert_with_ca(ca_crt: &X509, client_crt: &X509) -> Result<bool, String> {
    let now =
        Asn1Time::days_from_now(0).map_err(|e| format!("failed to get the time now :{}", e))?;
    if client_crt.not_before() > now || client_crt.not_after() < now {
        return Err("dates not valid".to_string());
    }
    let ca_pub_key = ca_crt
        .public_key()
        .map_err(|e| format!("failed to extract the CA public key : {}", e))?;
    client_crt
        .verify(&ca_pub_key)
        .map_err(|e| format!("certificate verficiation failed : {}", e))
}
...
pub async fn verify_signature_with_cert(
    ...
    let pkey = crt.public_key().map_err(|e| {
        format!(
            "failed to extract the public key from the provided cerificate : {}",
            e
        )
    })?;
    let mut verifier = Verifier::new(MessageDigest::sha256(), &pkey)
        .map_err(|e| format!("failed to instantiate the verifier : {}", e))?;
