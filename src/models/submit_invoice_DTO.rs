...
impl SubmitInvoiceDto {
    pub fn parse(self) -> Result<IntermediateInvoiceDto,String>{
      ...
      // let certificate_bytes = general_purpose::STANDARD.decode(self.certificate).map_err(|_| "invalid base64 certificate")?;
      let certificate = X509::from_pem(self.certificate.as_bytes()).map_err(|_| "invalid x509 certificate")?;
