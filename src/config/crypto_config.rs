...
impl Crypto {
    pub async fn from_env() -> Result<Self, String> {
        ...
        let private_key_base64 =
            env::var("SEC_PRIVATE_KEY").map_err(|e| format!("failed to read the private_key from env : {}",e))?;
        let certificate_base64 =
            env::var("SEC_CERTIFICATE").map_err(|e| format!("failed to read the certificate from env : {}",e))?;
